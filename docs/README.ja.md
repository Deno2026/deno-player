...
インストールせずに使いたい場合は、[Releases](https://github.com/Deno2026/deno-video-player/releases) から `DenoVideoPlayer-<version>-portable-win-x64.zip` という名前の最新ファイルをダウンロードし、解凍して `DenoVideoPlayer.exe` を実行してください。
